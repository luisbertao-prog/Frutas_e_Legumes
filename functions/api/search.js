...
function decode(v = "") { return v.replace(/&nbsp;/gi," ").replace(/&amp;/gi,"&").replace(/&quot;/gi,'"').replace(/&#39;/gi,"'").replace(/&euro;/gi,"€").replace(/&#x20AC;/gi,"€"); }
...
function walkJson(node, out, store, q, fallback){
  ...
  const name=clean(node.name||node.productName||node.title||node.displayName||node.item_name||"");
...
function structuredHtml(html, store, q, fallback){
  ...
  for(const b of blocks.slice(0,3000)){
    ...
    const name=text(nameRaw||plain.slice(0,180)); if(!relevant(name,q)) continue;
  ...
  while((pos=low.indexOf(query,pos))!==-1 && count<80){
    ...
    if(prices.length){
      ...
      if(validPrice(p)){
        ...
        let name=clean(chunk.slice(start,Math.min(chunk.length,(pos-before)+100))).replace(/^[-–|: ]+/,"");
        if(name.length>160) name=name.slice(-160); if(!relevant(name,q)) name=clean(chunk.slice(Math.max(0,pos-before-50),pos-before+90));
...
async function fetchStore(store,q){
  ...
  const seen=new Set(); const items=gathered.filter(x=>{
    x.name=clean(x.name); if(!relevant(x.name,q)||!validPrice(x.price))return false;
